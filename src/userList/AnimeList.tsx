...
import { AnimeWatchStatus, IUserAnimeEntry, PaginationResponse } from "../commonTypes";
...
export const AnimeList = () => {
   ...
   const updateEntryState = (animeEntry: IUserAnimeEntry) => {
      const updatedState: UserEntriesAPIResponse = {
   ...
   const deleteEntryState = (malId: string) => {
      const updatedState: UserEntriesAPIResponse = {
         pagination: data?.pagination as PaginationResponse,
         data: data?.data.filter((entry) => entry.malId !== malId) as IUserAnimeEntry[],
      };
   ...
   const handleIncrementPageNum = () => {
      if (currentPage !== data?.pagination.totalPages) setCurrentPage(currentPage + 1);
   };
   const handleCurrentSearchTermChange = () => {
      if (searchTermInput !== currentSearchTerm) setCurrentSearchTerm(searchTermInput);
   };

   const handleCurrentSearchTermChangeByEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Enter") handleCurrentSearchTermChange();
   };
   ...
   }, [statusFilter, currentPage, currentSearchTerm, navigate, logout, userInfo]);

   return (
      <>
         <Header />
         <section className="flex flex-col p-20 gap-14">
            <div className="flex flex-col md:flex-row md:items-stretch items-center  gap-4 justify-between">
